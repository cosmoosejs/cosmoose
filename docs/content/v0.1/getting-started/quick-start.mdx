...
<Tabs items={["pnpm", "yarn", "npm"]}>
  ...
  <Tab value="npm">
    ```bash
    npm install cosmoose
    ```
  </Tab>
</Tabs>

<Callout type="info">
  Also available as `@cosmoose/cosmoose` — both package names point to the same library.
</Callout>

The following dependencies are included automatically when you install `cosmoose`:
...
```ts title="schema.ts"
import { Schema, Type } from 'cosmoose';
