...
- [ ] 2.2 Implement `SchemaDefinition<T>` types — field descriptors for each `Type` with metadata (`unique`, `optional`, `default`, `trim`, `lowercase`, `uppercase`)
- [ ] 2.3 Implement `Document<T>` type including CosmosDB metadata (`id`, `_etag`, `_ts`, `_rid`, `_self`, `_attachments`)
...
- [ ] 4.1 Implement `Schema<T>` class constructor that stores definition, options, and collects unique keys
...
- [ ] 4.8 Implement `addCompositeUniqueKey()` and `addCompositeIndex()` with path validation
- [ ] 4.9 Implement accessors: `getPartitionKey()`, `getUniqueKeys()`, `getCompositeIndexes()`, `getTtl()`, `getDefinition()`
- [ ] 4.10 Write tests for all type compilations, schema variants, timestamps, unique keys, composite indexes
...
- [ ] 5.9 Implement `.exec()` dispatch to correct query execution method based on query type
...
- [ ] 8.1 Implement `syncContainers()` — iterate registered models and create containers with partition key, unique key policy, composite indexes, TTL, indexing policy via `createIfNotExists`
- [ ] 8.2 Implement `syncContainer(name)` — provision a single named container, error on unregistered name
- [ ] 8.3 Write tests for container provisioning with all configuration combinations
